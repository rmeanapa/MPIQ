...
!   & ";mkdir -p /scratch/meanapan" // &                                                   ! Create scratch directory if needed 
   & ";/bin/cp "//trim(adjustl(fname(i)))//".mop scratch-"//trim(adjustl(fname(i)))// &   ! Copy input file in the work directory
   & ";cd scratch-"//trim(adjustl(fname(i)))// &                                          ! Change to the work directory 
   & ";/home/truhlard/meanapan/soft/MOPAC/MOPAC2012.exe  " //trim(adjustl(fname(i)))//".mop > "//trim(adjustl(fname(i)))//".out "// & ! Run code 
